    ...
    <div id="good-table">
        ...
        <el-table class="table-list" :data="list" stripe height="600" @selection-change="handleSelectionChange">
            ...
            <el-table-column min-width="80" align="center" label="图片">
                <template scope="scope">
                    <img :src="imgbase_url + '/img_get.php?img=1&height=69&width=69&imgname=' + scope.row.food_img_list" alt="菜品图片">
                </template>
            </el-table-column>
            ...
            <el-table-column min-width="60" align="center" label="规格">
                <template scope="scope">
            ...
            <el-table-column min-width="80" align="center" label="当日销量">
                <template scope="scope">
                    <span>{{ scope.row.food_num_day}}</span>
                </template>
            </el-table-column>
            <el-table-column min-width="80" align="center" label="周销量">
                <template scope="scope">
                    <span>{{ scope.row.food_num_week}}</span>
                </template>
            </el-table-column>
            <el-table-column min-width="80" align="center" label="月销量">
                <template scope="scope">
            ...
            <el-table-column min-width="80" align="center" label="分享数">
                <template scope="scope">
                    <span>{{ scope.row.share_num}}</span>
                </template>
            </el-table-column>
            <el-table-column min-width="80" align="center" label="收藏数">
                <template scope="scope">
                    <span>{{ scope.row.favorites_num}}</span>
                </template>
            </el-table-column>
            <el-table-column min-width="80" align="center" label="评论数">
                <template scope="scope">
            ...
            <el-table-column min-width="140" align="center" label="操作">
                <template scope="scope">
                    <span @click="stateSingleOff(scope.row.food_id)" 
                      v-if="scope.row.sale_off === SaleStatusCfg.YES" 
                      class="red-text" 
                      style="padding-right:10px;padding-bottom:10px;">下架</span>
                    <span @click="stateSingleOn(scope.row.food_id)" 
                    v-else class="blue-text" style="padding-right:10px;padding-bottom:10px;">上架</span>
                    ...
                    <span class="blue-text">
                        ...
                        <router-link v-if="scope.row.type === goodType.DRINK" :to="{path:'/good/drinkeditor',query: { foodId:scope.row.food_id}}">
                            编辑
                        </router-link>
                    </span>
                    <span @click="deleteFood(scope.row.food_id)" 
                    class="red-text" 
                    style="padding-left:10px;padding-top:10px;">删除</span>
                </template>
            </el-table-column>
        </el-table>
        </div>
...
export default {
      ...
      saleStatusItem: [
        {
          ...
          label: SaleStatus.toString(SaleStatus.ALL)
        },
        {
          value: SaleStatus.YES,
          label: SaleStatus.toString(SaleStatus.YES)
        },
        {
          value: SaleStatus.NO,
  ...
  methods: {
    ...
    deleteFood(foodId) {
      let foodArr = [];
      foodArr.push(foodId);
      GoodList.deleteFoodItems(foodArr, this.getList);
    },
    //批量删除
    multipleDeleteFood() {
    ...
    stateSingleOn(foodId) {
      ...
      GoodList.changeStatusOn(foodArr, this.getList);
    },
    stateSingleOff(foodId) {
      let foodArr = [];
      foodArr.push(foodId);

      GoodList.changeStatusOff(foodArr, this.getList);
    },
    stateOn() {
      let foodArr = this.multipleSelection.map(function(item) {
        return item.food_id;
